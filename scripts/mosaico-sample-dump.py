"""Transfer the MCU-captured H.264 sample without decoding any video on the computer."""
import argparse
import base64
...
root=Path(__file__).resolve().parent.parent/'artifacts/mosaico/benchmarks'
root.mkdir(parents=True,exist_ok=True)
stamp=datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
log_path=root/(stamp+'-transfer.log');image_path=root/(stamp+'-sample.h264')
...
with serial.Serial(ports[0].device,115200,timeout=.2) as uart,log_path.open('xb') as log:
    ...
    while time.monotonic()<end:
        ...
        while b'\n' in pending:
            ...
            if b'OFT_SAMPLE_NOT_READY' in row:raise SystemExit('MCU has not retained a requested IDR yet.')
            match=re.search(rb'OFT_SAMPLE_BEGIN bytes=(\d+) sha256=([0-9a-f]{64})',row)
            if match:
                expected=int(match[1]);digest=match[2].decode()
                if expected>1024*1024:raise SystemExit('Refusing an oversized image transfer.')
            match=re.search(rb'^OFT_SAMPLE_DATA (\d+) ([A-Za-z0-9+/=]+)$',row)
            if match and expected is not None:
                ...
                blocks[index]=chunk
            if row==b'OFT_SAMPLE_END':finished=True;break
        if finished:break
...
print(f'SAMPLE={image_path}\nSHA256={digest}\nBytes={len(raw)}; H.264 received on Mosaico; host transferred bytes only.')
